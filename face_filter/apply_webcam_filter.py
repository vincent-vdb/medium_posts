        ...
        if not ret:
            ...
            if VISUALIZE_FACE_POINTS:
                ...
                for idx, point in enumerate(points2):
                    cv2.circle(viz, point, 2, (255, 0, 0), -1)
                    # cv2.putText(viz, str(idx), point, cv2.FONT_HERSHEY_SIMPLEX, .3, (255, 255, 255), 1)
            ...
            for idx, filter in enumerate(filters):
                ...
                if filter['morph']:
                    ...
                    if VISUALIZE_FACE_POINTS:
                        viz = np.copy(frame)
                        for triangle in dt:
                            landmarks = np.array([points2[i] for i in triangle])
                    ...
                    dst_points = [points2[int(list(points1.keys())[0])], points2[int(list(points1.keys())[16])], points2[int(list(points1.keys())[71])]]
            ...
            output = cv2.putText(output.astype(np.uint8), f"fps: {fps:.2f}", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, .5, (255, 0, 0), 1)
