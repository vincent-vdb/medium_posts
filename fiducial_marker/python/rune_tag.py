...
class Runetag:
    def __init__(
        self, num_layers: int = 3, num_dots_per_layer: int = 43, gap_factor: float = 1.3
    ):
    ...
    def get_keypoints_with_labels(self, keypoints):
        x_and_y_with_labels = [
            [markpoint.x, markpoint.y, label]
            for markpoint, label in zip(self.markpoints, keypoints)
        ]
    ...
    def draw_runetag(self, scale_in_pixel, keypoints):
        ...
        for radius, kpt_with_id in zip(radius_list, x_and_y_with_labels):
            ...
            Runetag.add_circle(
                image,
                x * scale_in_pixel + center,
                y * scale_in_pixel + center,
                radius * scale_in_pixel,
            )

        return 1 - image

    def generate_random_tag(
        self, pixel_size: int = 128, output_path: str = None, write_file: bool = True
    ):
        # Generate keypoint labels for tag generation
        codes = np.random.randint(
            0, 2**self.num_layers - 1, self.num_dots_per_layer - 1
        )
        keypoint_labels = Runetag.slot_codes_to_binary_ids(
            codes, num_layers=self.num_layers, is_outer_first=True
        )
