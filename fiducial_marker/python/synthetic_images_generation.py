...
def generate_synthetic_images_with_tag(
    ...
    """
    composition = [
        A.ColorJitter(brightness=[0.5, 1.0], p=0.1),
        A.Affine(
            scale=[0.3, 4.0],
            rotate=[-180, 180],
            shear=[-30, 30],
        ...
        A.Perspective(fit_output=True, p=0.9, pad_val=[250, 252, 254]),
        A.MotionBlur(),
        A.Defocus(radius=(1, 5)),
        A.RandomScale(scale_limit=(0, 0), p=1.0),
    ]
    transform = A.Compose(
        composition,
    ...
    while idx < n_images_to_generate:
        ...
        if background_images_path is None:
            ...
            mask = (
                (transformed_image[:, :, 0] == 250)
                & (transformed_image[:, :, 1] == 252)
                & (transformed_image[:, :, 2] == 254)
            )
            mask = np.expand_dims(mask, -1).astype(int)
            tmp = (
                transformed_image * (1 - mask) + bkg_image[i : i + h, j : j + w] * mask
            )
            # Compute output image
            bkg_image[i : i + h, j : j + w] = tmp
            transformed_labels[:, 0] = (
                transformed_labels[:, 0] * transformed_image.shape[1] + j
            ) / bkg_image.shape[1]
            transformed_labels[:, 1] = (
                transformed_labels[:, 1] * transformed_image.shape[0] + i
            ) / bkg_image.shape[0]
            transformed_labels[:, 2] = (
                transformed_labels[:, 2]
                * transformed_image.shape[0]
                / bkg_image.shape[0]
            )
            transformed_labels[:, 3] = (
                transformed_labels[:, 3]
                * transformed_image.shape[0]
                / bkg_image.shape[0]
            )
        ...
        try:
            output = final(
                image=bkg_image, bboxes=transformed_labels, class_labels=["tag"]
            )
        except ValueError:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate synthetic images and labels with rune tags"
    )
    parser.add_argument(
        "--number", type=int, help="number of synthetic images to generate", default=2
    )
    parser.add_argument(
        "--output_path",
        type=str,
    ...
    parser.add_argument(
        ...
        default="background_images/train/",
    )
    parser.add_argument(
